          ...
          <div className=''>
            <h1 className='text-3xl text-black bold mb-7 dark:text-white'>Writers List</h1>
              <div className='card-list flex flex-wrap'>
                  {
                      writerData.map((item)=>
                          <div key={item.id} className='writer text-center md:w-2/12 sm:w-3/12 flex flex-col items-center mx-2 my-4 p-7 shadow-sm shadow-gray-500 dark:bg-cardDark dark:text-white border-gray dark:border-borderDark dark:shadow-lg'>
                              <img src={item.imageUrl} alt={item.name} className='mb-3 rounded-full w-[90px] max-w-[91px] h-[90px] rounded-full w-[70px] h-[70px]'/>
